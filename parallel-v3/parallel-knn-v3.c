...
int main(int argc, char *argv[]) {
    ...
    double *my_points, *min_dist, *distances;
    int *near_neigh;
    ...
    MPI_Bcast(my_points, N * 3, MPI_DOUBLE, COORDINATOR, MPI_COMM_WORLD);


    for (i=0; i < numProcs; i++) {

        computeDistances(&my_points[rank*stripsize*3], &my_points[i*stripsize*3], distances, stripsize, i == rank);

        updateNeighbours(near_neigh, min_dist, K, distances, stripsize, i*stripsize, i == rank);
